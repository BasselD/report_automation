...
DATA_PATH = "/Users/bassel_instructor/Documents/datasets/medicare_synthetic_12k.parquet"
# Tip: also accepts CSV → DATA_PATH = r"\\shared_drive\...\file.csv"

# Column names mapped to the actual dataset schema
C = {
    "paid":        "TotalPaid",
...
<style>
    ...
    .kpi-card {
        ...
        text-align: center;
        border-top: 3px solid #4c9be8;
        margin-bottom: 6px;
        min-height: 95px;
    }
    .kpi-oon { border-top-color: #f6ad55; }
    .kpi-label {
        font-size: 0.72rem; color: #8899aa;
        text-transform: uppercase; letter-spacing: 0.06em;
        line-height: 1.3;
    }
    .kpi-value { font-size: 1.65rem; font-weight: 700; color: #e8f0fe; margin-top: 5px; }
    .block-container { padding-top: 1.5rem; }
    div[data-testid="stMetric"] { background: transparent; }
    .section-header { color: #8899aa; font-size: 0.78rem; text-transform: uppercase;
...
# ══════════════════════════════════════════════════════════════════
#  HELPER: detect file type and return appropriate read_* function
# ══════════════════════════════════════════════════════════════════
def read_expr(path: str) -> str:
    """Return DuckDB read expression based on file extension."""
...
# ══════════════════════════════════════════════════════════════════
#  DATA REGISTRATION — loads filter options (cached 1 hr)
# ══════════════════════════════════════════════════════════════════
...
def get_scatter(data_path: str, wc: str, gran_col: str) -> pd.DataFrame:
    expr = read_expr(data_path)
    # Benchmark = overall avg TotalPaid across the filtered dataset
    q = f"""
    WITH filtered AS (
        SELECT * FROM {expr} {wc}
    ),
...
# ── Data source config ──────────────────────────────────────────
with st.expander("⚙️ Data Source", expanded=not Path(DATA_PATH).exists()):
...
with st.sidebar:
    ...
    st.divider()
    st.markdown('<p class="section-header">📊 Scatter Granularity</p>', unsafe_allow_html=True)
    gran_label = st.selectbox("Group By", list(GRANULARITY_OPTIONS.keys()))
    gran_col   = GRANULARITY_OPTIONS[gran_label]

    st.markdown('<p class="section-header">🎨 Color Bubbles By</p>', unsafe_allow_html=True)
    color_by = st.radio("", ["Specialty", "Network Status"], horizontal=True, label_visibility="collapsed")
...
kpi_items = [
    ...
    ("🌐 % Out-of-Network Spend",
     f"{kpi['pct_oon']:.1f}%" if not pd.isna(kpi["pct_oon"]) else "—", "kpi-oon"),
]
...
if scatter_df.empty:
    ...
    fig.update_layout(
        height=560,
        plot_bgcolor="#151926",
        paper_bgcolor="#151926",
        font=dict(color="#c8d6e8"),
        legend=dict(orientation="v", x=1.01, bgcolor="rgba(0,0,0,0)"),
        xaxis=dict(showgrid=True, gridcolor="#252b40", zeroline=False),
        yaxis=dict(showgrid=True, gridcolor="#252b40", zeroline=False),
...
with dl_col3:
    st.markdown("<br>", unsafe_allow_html=True)
    run_dl = st.button("⬇️ Prepare Download", use_container_width=True)
...
# ── FOOTER ──────────────────────────────────────────────────────
st.divider()
st.caption("Professional Spend Analytics · DuckDB + Streamlit · Local Browser · © 2025")
