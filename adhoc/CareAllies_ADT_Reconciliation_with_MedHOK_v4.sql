/*=============================================================================
  ...
  - MedHOK authorization has no usable patient_class and is matched in a
    separate member/date branch with ambiguity controls.
...
SELECT
    ...
    /* Clinical ADT/HIE/EMR matching tolerance. */
    1::INTEGER AS adt_fuzzy_days,

    /* MedHOK authorization matching tolerance. */
    1::INTEGER AS medhok_fuzzy_days,

    /*
       Set to 0 if the business confirms MedHOK does not represent ED events.
       Set to 1 to allow ED authorization candidates subject to ambiguity rules.
    */
...
/*=============================================================================
  5. CLINICAL ADT / HIE / EMR ADMISSION MATCHING

  MedHOK authorization is intentionally excluded from this branch because its
  patient_class is blank and its administrative dates require different
  matching and ambiguity controls.

  Approved clinical patient-class rule:
  - ED claims: ED or missing ADT patient class
  - IP claims: IP or missing ADT patient class
  - OBS claims: OBS preferred; missing class, ED, or IP allowed as fallbacks
=============================================================================*/
...
CREATE TEMP TABLE tmp_adt_admission_match_candidates AS
SELECT
    e.claim_event_id,
    e.native_event_id,
    e.claims_source,
    ...
    CASE
        ...
        ELSE 'Fuzzy Admission Date +/-'
             || CAST(p.adt_fuzzy_days AS VARCHAR(10))
             || ' Day'
    END AS match_method,
    ...
    CASE
        ...
        WHEN e.care_type = 'OBS' AND a.patient_class IN ('ED', 'IP')
...
CROSS JOIN tmp_params p
WHERE ABS(DATEDIFF(day, e.event_admit_date, a.admit_date))
      <= p.adt_fuzzy_days;
...
WHERE claim_feed_rn = 1;

/*=============================================================================
  6. CLINICAL ADT / HIE / EMR DISCHARGE MATCHING
=============================================================================*/
...
CREATE TEMP TABLE tmp_adt_discharge_match_candidates AS
SELECT
    e.claim_event_id,
    e.native_event_id,
    e.claims_source,
...
/*=============================================================================
  7. SEPARATE MEDHOK AUTHORIZATION BRANCH

  MedHOK has no usable patient_class. It is matched using:
  - Same person
  - Admission or discharge date within the MedHOK tolerance
  - Exact admit/discharge date pair when available
  - Facility agreement when available
  - Unique exact-date evidence
  - Explicit ambiguity controls

  Ambiguous same-member/same-date candidates are excluded from production
  coverage unless a unique date-pair or facility signal resolves the event.
=============================================================================*/
...
CREATE TEMP TABLE tmp_medhok_admission_candidates AS
...
SELECT
    b.*,
    CASE
        WHEN b.exact_admit_discharge_pair_flag = 1 THEN 1
        WHEN b.exact_target_date_flag = 1 AND b.facility_match_flag = 1 THEN 2
        WHEN b.exact_target_date_flag = 1 THEN 3
        WHEN b.facility_match_flag = 1 THEN 4
        ELSE 5
...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN exact_target_date_flag = 1 THEN claim_event_id END)
        AS exact_date_candidate_count,
    COUNT(DISTINCT CASE WHEN exact_admit_discharge_pair_flag = 1 THEN claim_event_id END)
        AS exact_pair_candidate_count,
    COUNT(DISTINCT CASE WHEN facility_match_flag = 1 THEN claim_event_id END)
        AS facility_supported_candidate_count
...
SELECT
    ...
    'ADMISSION' AS match_target,
    CASE
        WHEN date_difference = 0
            THEN 'Exact Authorization Admission Date'
...
CREATE TEMP TABLE tmp_medhok_discharge_candidates AS
WITH candidate_base AS
(
    SELECT
        e.claim_event_id,
        ...
        CASE
            WHEN a.admit_date IS NOT NULL
             ...
             AND e.event_discharge_date = a.discharge_match_date
                THEN 1
            ELSE 0
        END AS exact_admit_discharge_pair_flag,
...
SELECT
    ...
    CASE
        WHEN date_difference = 0
            THEN 'Exact Authorization Discharge Date'
...
/*=============================================================================
  8. COMBINE CLINICAL AND MEDHOK SELECTED MATCHES

  One row per claim event + notification target + selected source/feed.
=============================================================================*/
...
SELECT
    ...
    first_admission_event_location,
    ...
    admission_pipeline_lag_minutes,
    admission_match_method,
    ...
    first_admission_authorization_candidate_count,
    first_admission_authorization_match_score
FROM
(
    SELECT
        claim_event_id,
        ...
        authorization_match_status AS first_admission_authorization_match_status,
        authorization_candidate_event_count AS first_admission_authorization_candidate_count,
        authorization_match_score AS first_admission_authorization_match_score,
        ROW_NUMBER() OVER
        (
            ...
            ORDER BY
                ...
                source_category,
                source_feed_key
        ) AS first_notification_rn
    FROM analytics.notification_match_detail_2025
    WHERE match_target = 'ADMISSION'
) ranked
WHERE first_notification_rn = 1;
...
SELECT
    ...
    first_discharge_event_location,
    ...
    discharge_pipeline_lag_minutes,
    discharge_match_method,
    ...
    first_discharge_authorization_candidate_count,
    first_discharge_authorization_match_score
FROM
(
    SELECT
        claim_event_id,
        ...
        authorization_match_status AS first_discharge_authorization_match_status,
        authorization_candidate_event_count AS first_discharge_authorization_candidate_count,
        authorization_match_score AS first_discharge_authorization_match_score,
        ROW_NUMBER() OVER
        (
            PARTITION BY claim_event_id
            ORDER BY
...
SELECT
    ...
    CASE
        WHEN COALESCE(a.admission_notification_flag, 0) = 0
            THEN 'No Matched Admission Notification'
    ...
    CASE
        WHEN COALESCE(a.discharge_notification_flag, 0) = 0
            THEN 'No Matched Discharge Notification'
    ...
    CASE
        ...
        ELSE 0
    END AS no_matched_notification_flag,
    ...
    fa.first_admission_authorization_match_status,
    fa.first_admission_authorization_candidate_count,
    fa.first_admission_authorization_match_score,
    ...
    fd.first_discharge_authorization_match_status,
    fd.first_discharge_authorization_candidate_count,
    fd.first_discharge_authorization_match_score,
...
SELECT
    ...
    SUM(no_matched_notification_flag) AS events_with_no_matched_notification,
    ROUND
    (
        100.0 * SUM(no_matched_notification_flag) / NULLIF(COUNT(*), 0),
        2
    ) AS percent_no_matched_notification,
...
/* QA 2: Confirm the four exclusive categories add to total events. */
SELECT
    care_type,
    COUNT(*) AS total_events,
    SUM(CASE WHEN notification_completeness_status = 'Admission and Discharge Received' THEN 1 ELSE 0 END)
        AS complete_covered,
    SUM(CASE WHEN notification_completeness_status = 'Admission Only' THEN 1 ELSE 0 END)
        AS admission_only,
    SUM(CASE WHEN notification_completeness_status = 'Discharge Only' THEN 1 ELSE 0 END)
        AS discharge_only,
    SUM(CASE WHEN notification_completeness_status = 'No Matched Notification' THEN 1 ELSE 0 END)
        AS no_matched_notification
FROM analytics.notification_reconciliation_2025
GROUP BY care_type
ORDER BY care_type;

/* QA 3: Clinical patient-class matching composition. */
SELECT
    match_target,
    care_type,
    patient_class_match_status,
    COUNT(*) AS matched_rows,
    COUNT(DISTINCT claim_event_id) AS matched_events
FROM analytics.notification_match_detail_2025
...
/* QA 4: MedHOK selected matches by target, care setting, and resolution method. */
SELECT
    match_target,
    care_type,
    authorization_match_status,
    COUNT(*) AS selected_rows,
    COUNT(DISTINCT claim_event_id) AS selected_events
FROM analytics.notification_match_detail_2025
WHERE source_category = 'AUTHORIZATION'
GROUP BY
    match_target,
...
/* QA 5: MedHOK candidate waterfall. */
SELECT
    'Admission candidates' AS stage,
    COUNT(*) AS row_count,
    COUNT(DISTINCT claim_event_id) AS claim_event_count,
    COUNT(DISTINCT adt_message_key) AS message_count
FROM tmp_medhok_admission_candidates
...
FROM tmp_medhok_admission_assignment
...
SELECT
    ...
    COUNT(DISTINCT claim_event_id),
    COUNT(DISTINCT adt_message_key)
FROM tmp_medhok_admission_matches
...
SELECT
    ...
    COUNT(DISTINCT claim_event_id),
    COUNT(DISTINCT adt_message_key)
FROM tmp_medhok_discharge_assignment
...
SELECT
    ...
    COUNT(DISTINCT claim_event_id),
    COUNT(DISTINCT adt_message_key)
FROM tmp_medhok_discharge_matches;

/* QA 6: Count MedHOK messages excluded because they remained ambiguous. */
SELECT
    'ADMISSION' AS match_target,
    COUNT(*) AS ambiguous_assigned_messages
FROM tmp_medhok_admission_assignment a
WHERE NOT
(
       a.candidate_event_count = 1
    OR (a.exact_admit_discharge_pair_flag = 1 AND a.exact_pair_candidate_count = 1)
    OR (a.facility_match_flag = 1 AND a.facility_supported_candidate_count = 1)
    OR (a.exact_target_date_flag = 1 AND a.exact_date_candidate_count = 1)
)
...
/* QA 7: Authorization flags should now be visible in the final table. */
SELECT
    care_type,
    COUNT(*) AS total_events,
    SUM(admission_authorization_flag) AS admission_authorization_events,
    SUM(discharge_authorization_flag) AS discharge_authorization_events,
    SUM(authorization_flag) AS any_authorization_events
FROM analytics.notification_reconciliation_2025
GROUP BY care_type
ORDER BY care_type;

/* QA 8: Verify a selected message is not assigned to multiple claim events for the same target. */
SELECT
    match_target,
    adt_message_key,
    COUNT(DISTINCT claim_event_id) AS matched_claim_events
FROM analytics.notification_match_detail_2025
GROUP BY
    match_target,
    adt_message_key
HAVING COUNT(DISTINCT claim_event_id) > 1
ORDER BY
    matched_claim_events DESC,
    match_target,
    adt_message_key;

/* QA 9: Review source classifications and sending-source names. */
SELECT
    source_category,
    sending_source,
    COUNT(*) AS matched_rows,
