...
================================================================================
  PURPOSE
  -------
  Build an event-level IP/OBS/LTAC authorization backbone with attending provider,
  facility, hospitalist group, first inpatient consult, observation-claim indicator, provider-volume flags,
...
/*===============================================================================
  04B. CONSULT SPECIALTY PRIORITY MAP
       Maintains the consult definition outside the ranking logic.

       Notes:
       - Add or revise Level3Bucket values based on the actual source taxonomy.
       - Lower ConsultPriority values win only when service dates tie.
       - The first qualifying consult DOS during the stay is selected first.
===============================================================================*/

CREATE MULTISET VOLATILE TABLE vt_consult_specialty_map
(
      Level3BucketMatch VARCHAR(100)
    , ConsultServiceGroup VARCHAR(100)
    , ConsultPriority BYTEINT
)
PRIMARY INDEX (Level3BucketMatch)
ON COMMIT PRESERVE ROWS
;

INSERT INTO vt_consult_specialty_map VALUES ('Cardiology',                'Cardiology',               1);
INSERT INTO vt_consult_specialty_map VALUES ('Nephrology',                'Nephrology',               2);
INSERT INTO vt_consult_specialty_map VALUES ('Pulmonary Disease',         'Pulmonary / Critical Care',3);
INSERT INTO vt_consult_specialty_map VALUES ('Infectious Disease',        'Infectious Disease',       4);
INSERT INTO vt_consult_specialty_map VALUES ('Gastroenterology',          'Gastroenterology',         5);
INSERT INTO vt_consult_specialty_map VALUES ('Neurology',                 'Neurology',                6);
INSERT INTO vt_consult_specialty_map VALUES ('Hematology/Oncology',       'Hematology / Oncology',    7);
INSERT INTO vt_consult_specialty_map VALUES ('Oncology',                  'Hematology / Oncology',    7);
INSERT INTO vt_consult_specialty_map VALUES ('General Surgery',           'General Surgery',          8);
INSERT INTO vt_consult_specialty_map VALUES ('Orthopedic Surgery',        'Orthopedic Surgery',       9);
INSERT INTO vt_consult_specialty_map VALUES ('Psychiatry',                'Psychiatry',              10);
INSERT INTO vt_consult_specialty_map VALUES ('Endocrinology',             'Endocrinology',           11);
INSERT INTO vt_consult_specialty_map VALUES ('Urology',                   'Urology',                 12);
INSERT INTO vt_consult_specialty_map VALUES ('Vascular Surgery',          'Vascular Surgery',        13);
INSERT INTO vt_consult_specialty_map VALUES ('Thoracic Surgery',          'Thoracic Surgery',        14);
INSERT INTO vt_consult_specialty_map VALUES ('Palliative Care',           'Palliative Care',         15);

COLLECT STATISTICS COLUMN (Level3BucketMatch) ON vt_consult_specialty_map;
...
/*===============================================================================
  05B. FIRST QUALIFYING CONSULT PER AUTHORIZATION STAY
       Grain: one row per AuthznKey.

       Selection rules:
       1. Service date must fall within the inpatient stay.
       2. Provider must match a consult specialty in vt_consult_specialty_map.
       3. Provider NPI must differ from the selected attending provider NPI.
       4. Earliest consult service date wins.
       5. Consult priority and NPI break same-day ties.
===============================================================================*/

CREATE MULTISET VOLATILE TABLE vt_consult_ranked AS
(
    SELECT
          IP.AuthznKey
        , HP.Level1Bucket
        ...
        , ROW_NUMBER() OVER
          (
              ...
              ORDER BY
                  HP.DOSBegin ASC,
                  CSM.ConsultPriority ASC,
                  HP.ProviderNPI ASC
          ) AS ConsultRank
    FROM vt_ip_base AS IP
    INNER JOIN vt_hospitalist_claims AS HP
        ON IP.MemberID = HP.MemberID
       AND HP.DOSBegin BETWEEN IP.Admit AND COALESCE(IP.Discharge, IP.Admit)
    INNER JOIN vt_consult_specialty_map AS CSM
        ON UPPER(TRIM(HP.Level3Bucket)) = UPPER(TRIM(CSM.Level3BucketMatch))
    LEFT JOIN vt_attending_best AS ATN
        ON IP.AuthznKey = ATN.AuthznKey
    WHERE HP.ProviderNPI IS NOT NULL
      AND (
            ATN.ProviderNPI IS NULL
            OR HP.ProviderNPI <> ATN.ProviderNPI
          )
) WITH DATA
PRIMARY INDEX (AuthznKey)
ON COMMIT PRESERVE ROWS
...
CREATE MULTISET VOLATILE TABLE vt_consult_first AS
(
    SELECT *
    FROM vt_consult_ranked
    WHERE ConsultRank = 1
) WITH DATA
PRIMARY INDEX (AuthznKey)
ON COMMIT PRESERVE ROWS
...
COLLECT STATISTICS COLUMN (AuthznKey)   ON vt_consult_first;
COLLECT STATISTICS COLUMN (ProviderNPI) ON vt_consult_first;
COLLECT STATISTICS COLUMN (ProviderTIN) ON vt_consult_first;
...
(
    SELECT
        ...
        , CASE WHEN CS.ProviderNPI IS NOT NULL THEN 1 ELSE 0 END AS ConsultFlag
        , CS.ConsultServiceGroup
        , CS.Level1Bucket AS ConsultProviderLevel1Bucket
        , CS.Level2Bucket AS ConsultProviderLevel2Bucket
        , CS.Level3Bucket AS ConsultProviderLevel3Bucket
        , CS.DOSBegin AS ConsultServiceDate
        , CS.ProviderName AS ConsultProviderName
        , CS.ProviderNPI AS ConsultProviderNPI
        , CS.ProviderTIN AS ConsultProviderTIN
        , CS.ProviderSpecialtyCode AS ConsultProviderSpecialtyCode
        , CS.ProviderSpecialtyDesc AS ConsultProviderSpecialtyDesc
        , CS.ProviderTaxonomyCode AS ConsultProviderTaxonomyCode
        , CS.ProviderTaxonomyCodeDesc AS ConsultProviderTaxonomyCodeDesc
        , CS.ConsultPriority AS ConsultSpecialtyPriority
    ...
    LEFT JOIN vt_consult_first AS CS
        ON IP.AuthznKey = CS.AuthznKey
...
COLLECT STATISTICS COLUMN (ConsultProviderNPI) ON vt_event_backbone_pre_volume;
COLLECT STATISTICS COLUMN (ConsultServiceGroup) ON vt_event_backbone_pre_volume;
...
COLLECT STATISTICS COLUMN (ConsultProviderNPI) ON vt_event_backbone;
COLLECT STATISTICS COLUMN (ConsultServiceGroup) ON vt_event_backbone;
...
(
    SELECT
        ...
        , COUNT(DISTINCT CASE WHEN BedType = 'LTAC' THEN AuthznKey END) AS LTACStayCount
        , SUM(CASE WHEN ConsultFlag = 1 THEN 1 ELSE 0 END) AS StayWithConsultCount
...
/* 13E. Consult capture QA. */
SELECT
      ConsultServiceGroup
    , COUNT(*) AS StayWithFirstConsultCount
    , COUNT(DISTINCT ConsultProviderNPI) AS DistinctConsultProviderCount
FROM vt_event_backbone
WHERE ConsultFlag = 1
GROUP BY 1
ORDER BY 2 DESC
...
/* 13F. Bed type QA, including LTAC. */
SELECT
      BedType
    , COUNT(DISTINCT AuthznKey) AS StayCount
FROM vt_event_backbone
...
/* 13G. Provider/facility monthly mart preview. */
SELECT *
FROM vt_mart_provider_facility_monthly
